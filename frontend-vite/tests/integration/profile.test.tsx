...
describe("Boundary Profile Test", () => {
  ...
  test("should show toast when major is not selected", async () => {
    render(
      <>
        <Toaster />
        <App />
    ...
    await screen.findByText("Major: Not Set");
    fireEvent.click(screen.getByRole("button", { name: "Edit Profile" }));
    fireEvent.click(screen.getByRole("button", { name: /Save/i }));
    await waitFor(() => {
      expect(screen.getByText("Please select a major.")).toBeVisible();
    });
    fireEvent.click(screen.getByLabelText(/Major/i));
    fireEvent.click(screen.getByRole("option", { name: "👩‍💻 Computer Science" }));
    fireEvent.click(screen.getByRole("button", { name: /Save/i }));
    await waitFor(() => {
