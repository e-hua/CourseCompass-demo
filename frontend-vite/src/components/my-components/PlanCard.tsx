...
interface PlanCardProps {
  ...
  courseNodeTypes: {
    CourseNode: React.MemoExoticComponent<React.FC<TooltipNodeProps>>;
  };
}
  ...
  const allNodes: Node[] = [
    ...[1, 2, 3, 4, 5, 6, 7, 8].map((i) => ({
      ...
      data: {
        label: (
          <div>
            {"Y" + Math.floor((i + 1) / 2) + "S" + (i % 2 === 0 ? 2 : 1)}
          </div>
        ),
      },
      width: 2400,
  ...
  return (
    <Card className="h-full w-full rounded border">
      ...
      >
        ...
        <Controls
          className="bg-white text-black border border-gray-300 shadow"
